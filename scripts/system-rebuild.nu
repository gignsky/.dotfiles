...
# NixOS system rebuild script with logging
...
def main [host?: string] {
    ...
    let log_file = $"rebuild-logs/nixos-rebuild-($target_host)-($timestamp).log"
    
    # Ensure log directory exists
    mkdir rebuild-logs
    
    # Run nixos-rebuild with logging
    let rebuild_result = (run-external "sudo" "nixos-rebuild" "switch" "--flake" $".#($target_host)" | complete)
    
    # Save output to log file
    let log_content = [
        $"# NixOS Rebuild Log: ($timestamp)",
        $"# Host: ($target_host)",
        $"# Command: sudo nixos-rebuild switch --flake .#($target_host)",
    ...
    if $rebuild_result.exit_code == 0 {
        print $"(ansi green)✅ NixOS rebuild successful(ansi reset)"
        
        # Get generation info for commit message
        let gen_info = (run-external "nixos-rebuild" "list-generations" | complete)
        let current_gen = if $gen_info.exit_code == 0 {
            $gen_info.stdout | lines | where { |line| $line | str contains "current" } | get 0?
        } else {
            $"generation info unavailable"
        }
        
        # Commit changes with generation info
        let commit_msg = $"($target_host): ($timestamp) -> ($current_gen)"
        ...
        print $"(ansi red)❌ NixOS rebuild failed with exit code ($rebuild_result.exit_code)(ansi reset)"
