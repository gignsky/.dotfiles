...
CUSTOM_CONFIG="hosts/spacedock/hardware-config.nix"
HOST_DEFAULT="hosts/spacedock/default.nix"
...
if nix build ".#nixosConfigurations.spacedock.config.system.build.toplevel" --dry-run >/dev/null 2>&1; then
  print_success "Configuration builds successfully"
else
  print_error "Configuration has build errors!"
  print_warning "Run: nix build '.#nixosConfigurations.spacedock.config.system.build.toplevel' for details"
  exit 1
fi
