{
  ...
  outputs =
    ...
    in
    {
      ...
      # Core configuration checks only - fast validation for CI/CD
      ...
      # Development checks - comprehensive validation including VM tests, packages, and pre-commit
      devChecks = {
        ${system} = import ./nix/devChecks.nix {
          inherit
            inputs
            outputs
            nixpkgs
      ...
      # Development shell configured with packages for working on nix-config
