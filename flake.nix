      ...
      pre-commit-check = inputs.pre-commit-hooks.lib.${system}.run {
        ...
        hooks = {
          ...
          scotty-post-commit-log = {
            enable = true; # Re-enabled with auto-commit functionality and loop prevention
            ...
            args = [
              ...
              ''
                                ...
                                if echo "$commit_message" | grep -q "^\📊 Scotty: Auto-log"; then
                                  echo "Skipping logger commit to prevent infinite loop"
                                  exit 0
                                fi
                                
                                # Log the original user commit
                                ...
                                if ! git diff-index --quiet HEAD -- scottys-journal/ 2>/dev/null; then
                                  ...
                                  original_commit=$(git log -1 --oneline | cut -d' ' -f1)
                                  git commit -m "📊 Scotty: Auto-log commit activity

                🤖 AUTOMATED ENGINEERING LOG COMMIT 🤖
                Generated by post-commit hook for engineering records.
                Original user commit: $original_commit
                Timestamp: $(date '+%Y-%m-%d %H:%M:%S')

                This commit was automatically created by Captain Scott's
                engineering logging system and should not trigger further
                automatic commits."
                                fi
              ''
            ];
